...
class SimpleSearchableRepositoryFactory<M, ID extends Serializable> extends JpaRepositoryFactory {
	//日志对象
	private static final Logger LOG = LoggerFactory.getLogger(SimpleSearchableRepositoryFactory.class);
	...
	private Object doGetTargetRepository(RepositoryMetadata metadata){
		...
		SimpleSearchableRepository<M, ID> repository = new SimpleSearchableRepository<M, ID>(entityInformation, entityManager);
